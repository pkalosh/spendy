...
def dashboard(request):
    ...
    context = {
        ...
        "total_in": total_deposits,
        "total_out": total_withdrawals,
...
def b2c_result_callback(request):
    ...
    and additional data extraction (TransactionReceipt, ReceiverPartyPublicName, phone number)
    """
    try:
        ...
        with transaction.atomic():
            ...
            amount_paid = None
            receiver_phone = None  # To store extracted phone number
            ...
            for param in result_parameters:
                ...
                if key == 'TransactionAmount':
                    ...
                    transaction_record.mpesa_code = value  # e.g., TLI851BK76
                    logger.debug(f"TransactionReceipt (Mpesa code): {value}")

                elif key == 'ReceiverPartyPublicName':
                    full_name_info = value  # e.g., "254768952248 - Patrick Kioko mutuku"
                    transaction_record.receiver_party_public_name = full_name_info
                    logger.debug(f"ReceiverPartyPublicName: {full_name_info}")

                    # Extract phone number: usually first part before " - "
                    if ' - ' in full_name_info:
                        potential_phone = full_name_info.split(' - ', 1)[0].strip()
                        # Basic validation: starts with 254 and 12 digits
                        if potential_phone.startswith('254') and len(potential_phone) == 12 and potential_phone.isdigit():
                            receiver_phone = potential_phone
                            # Optionally save to a dedicated field (add this field to your Transaction model)
                            transaction_record.phone_number = receiver_phone
                            logger.debug(f"Extracted receiver phone: {receiver_phone}")

                # You can keep these if needed, or remove if redundant
                elif key == 'SenderPartyPublicName':
                    transaction_record.sender_party_public_name = value

                elif key == 'TransactionCompletedDateTime':
                    # Optional: parse and save completion time more accurately
                    pass
            ...
            if result_code == 0:  # Success
                ...
                # Update fee transaction (existing logic)
                try:
                    ...
                    logger.error(f"Error updating fee transaction: {e}", exc_info=True)

                # Deduct from wallet (existing logic unchanged)
                if transaction_record.sender_wallet:
                    try:
                        ...
                        if wallet.balance >= total_deduction:
                            ...
                            pass  # (your existing handling)
                    except Exception as e:
                        logger.error(f"Error updating wallet: {e}", exc_info=True)

                # Mark expense as paid (existing logic)
                ...
                # Update fee transaction on failure, reset expense, etc. (your existing logic)

            # Common updates for MpesaTransaction
            mpesa_transaction.result_code = result_code
            mpesa_transaction.result_desc = result_desc
...
def b2b_result_callback(request):
    ...
    Now saves: ReceiverPartyPublicName, BillReferenceNumber, TransactionID (as mpesa_code)
    """
    try:
        result_data = json.loads(request.body)
        ...
        originator_conversation_id = result.get('OriginatorConversationID')
        conversation_id = result.get('ConversationID')
        transaction_id = result.get('TransactionID')  # e.g., TLJSG5LEQM
        ...
        with transaction.atomic():
            amount_paid = None
            bill_reference_number = None

            # Extract from ReferenceData (for BillReferenceNumber)
            reference_data = result.get('ReferenceData', {}).get('ReferenceItem', [])
            if isinstance(reference_data, list):
                for item in reference_data:
                    if isinstance(item, dict) and item.get('Key') == 'BillReferenceNumber':
                        bill_reference_number = item.get('Value')
                        transaction_record.phone_number = bill_reference_number
                        logger.debug(f"BillReferenceNumber saved: {bill_reference_number}")
            ...
            for param in result_parameters:
                ...
                if key == 'Amount':
                    try:
                        ...
                        mpesa_transaction.amount = amount_paid
                        transaction_record.amount = amount_paid  # Ensure main transaction has correct amount
                    except (ValueError, TypeError):
                    ...
                    logger.debug(f"ReceiverPartyPublicName saved: {value}")

                elif key == 'TransCompletedTime':
                    # Optional: parse datetime if you have a field for it
                    transaction_record.completed_at = value

                # You can add more fields here if needed later

            # Save TransactionID as mpesa_code (common reference/receipt field)
            if transaction_id:
                transaction_record.mpesa_code = transaction_id
                logger.debug(f"TransactionID saved as mpesa_code: {transaction_id}")
            ...
            if result_code == 0:  # Success
                ...
                if transaction_record.sender_wallet:
                    try:
                        ...
                        if wallet.balance >= total_deduction:
                            ...
                            logger.error(f"Insufficient funds in wallet {wallet.pk}")
                            # Handle insufficient funds (your existing logic)
                ...
                if hasattr(transaction_record, 'expense') and transaction_record.expense:
                    expense = transaction_record.expense
                    expense.paid = True
                    expense.paid_at = timezone.now()
                    expense.payment_completed = True
                ...
                mpesa_transaction.status = 'FAILED'
                logger.warning(f"B2B payment failed. Reason: {result_desc}")
                transaction_record.status = "failed"
                transaction_record.failure_reason = result_desc
                transaction_record.completed_at = timezone.now()
                transaction_record.save()

                # Update fee on failure
                try:
                    fee_transaction = TransactionFee.objects.get(
                        parent_transaction=transaction_record,
                        transaction_type="fee"
